    ...
    def _execute_web_search(self, data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                ...
                'query_used': query,
                'total_results': len(search_results.get('results', [])) if search_results else 0
            }
    ...
    def _consolidate_all_results(
        ...
        consolidated = {
            ...
            'consolidation_metadata': {
                'total_orchestrators_used': len([k for k in results.keys() if 'orchestrator' in k]),
                'components_completed': len([k for k, v in results.items() if not v.get('error')]),
...
"""
    ...
    def get_execution_status(self, session_id: str) -> Dict[str, Any]:
        ...
        with self.sync_lock:
            return self.execution_state.get(session_id, {'status': 'not_found'})
